...
/**
 * Resolve a manifest-style slug (e.g. "flux-dev", "veo-3.1-fast") to a Hedra
 * model UUID by matching against display_name.
 *
 * Matching normalizes both sides by lowercasing and stripping spaces/dashes,
 * so "flux-dev" matches "FLUX Dev" and "veo-3.1-fast" matches "Veo 3.1 Fast".
 *
 ...
 *
 * Throws with the list of available display_names when nothing matches.
 */
export function resolveModelId(slugOrUuid: string, models: readonly HedraModel[]): string {
  ...
  const hit = models.find((m) => normalize(m.display_name).includes(needle));
  if (hit) return hit.id;
  const names = models.map((m) => m.display_name).join(', ');
  throw new Error(
    `Model slug "${slugOrUuid}" did not match any Hedra model. Available: ${names}. ` +
    `Update manifest.json to use one of these display names (or its dashed/lowercase equivalent).`,
  );
}
